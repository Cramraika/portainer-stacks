...
- **Currently live:** `stacks/immich/` + `stacks/uptime-kuma/` (Git-backed on `vagary-core-1`; immich stack id 18 per live BoltDB probe 2026-06-08 — ids drift on recreate, see playbook §7 expected-output). (Reconciled 2026-06-07; id re-probed 2026-06-08 after the ML-split recreate.)
...
|---|---|---|---|
| **Portainer** (stack host) | `stacks/immich/` + `stacks/uptime-kuma/` are Git-backed Portainer stacks on **vagary-core-1** (immich id 18 per live BoltDB probe 2026-06-08; ids drift on recreate), hourly auto-update poll | Stack ops via Portainer API at `portainer.chinmayramraika.in`; never paste secrets into the Portainer UI env panel | `service-playbooks/substrate/portainer.md` |
