...
- Live Portainer (`vagary-core-1`) currently has one Git-backed stack: `immich-app` (above). `anjaan_online` was **removed** as a Portainer stack — it is Coolify-deployed (the former `stacks/anjaan_online/` was dropped). `stacks/uptime-kuma/` is **prepared but not yet migrated** to a Git-backed Portainer stack (container runs raw today). `cronicle` / `dns-proxy` from the early discovery inventory were **never created as Portainer stacks** — not pending exports. Canonical inventory: `platform-docs/02-governance/service-playbooks/substrate/portainer.md`.
