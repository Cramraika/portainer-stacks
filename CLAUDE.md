...
- CLAUDE.md preamble at universal **v53** (in sync; see the managed VERSION tag above, which is authoritative and self-updates via sync-preambles.py); VPS-infra + brand-registry + commercial-bound cluster playbooks are read-on-demand pointers per §49.10 (v50), not inlined
